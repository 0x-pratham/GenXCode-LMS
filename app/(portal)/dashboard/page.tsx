...
import { Swords, BookOpen, Video, ArrowRight, Activity, Flame, Terminal, Clock, Megaphone, Loader2 } from "lucide-react";
...
async function getDashboardData(userId: string) {
  ...
  const [
    { data: profile },
    { data: league },
    { data: totalXpSum }, // Alternative XP Fallback
    { data: quests },
    { data: courses },
    { data: nextSession },
    { data: announcements },
    ...
    // Aggregation query fallback to ensure we ALWAYS get accurate XP 
    // even if league_memberships row is delayed in creation.
    supabase.from("xp_events").select("amount").eq("user_id", userId),
    
    supabase.from("daily_challenges").select("*").eq("status", "published").order("created_at", { ascending: false }).limit(2),
    supabase.from("courses").select("*").eq("status", "published").order("created_at", { ascending: false }).limit(2),
    supabase.from("live_sessions").select("*").eq("status", "published").gte("starts_at", now).order("starts_at", { ascending: true }).limit(1).maybeSingle(),
    supabase.from("announcements").select("*").eq("status", "published").order("created_at", { ascending: false }).limit(3),
    supabase.from("xp_events").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(5)
  ]);

  return { profile, league, totalXpSum, quests, courses, nextSession, announcements, recentXpEvents };
}
...
export default async function DashboardPage() {
  ...
  // 2. Fetch all database data concurrently
  const { profile, league, totalXpSum, quests, courses, nextSession, announcements, recentXpEvents } = await getDashboardData(user.id);

  // 3. Logic to display accurate XP
  let calculatedXp = 0;
  if (totalXpSum && totalXpSum.length > 0) {
    calculatedXp = totalXpSum.reduce((acc, curr) => acc + (curr.amount || 0), 0);
  }
  
  // Preference: Use League table XP. If it's 0 or missing, fallback to actual calculated XP.
  const finalXpToDisplay = league?.xp_total ? league.xp_total : calculatedXp;
  ...
  return (
    <Suspense fallback={<DashboardSkeleton />}>
      <div className="space-y-8 max-w-7xl mx-auto pb-12 relative z-10 focus:outline-none" tabIndex={0}>
          ...
          <div className="relative z-10 flex flex-wrap sm:flex-nowrap gap-4 w-full lg:w-auto">
            <div className="flex-1 sm:flex-none bg-black/40 backdrop-blur-md rounded-2xl p-5 border border-white/10 text-center min-w-[150px] shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
              ...
              <div className="flex justify-center items-center gap-2 font-heading text-3xl font-bold text-foreground drop-shadow-md">
                <Flame className="w-6 h-6 text-orange-400" /> {finalXpToDisplay.toLocaleString()}
              </div>
            </div>
            <div className="flex-1 sm:flex-none bg-black/40 backdrop-blur-md rounded-2xl p-5 border border-white/10 text-center min-w-[150px] shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="space-y-5 animate-fade-in-up [animation-delay:200ms] opacity-0 fill-mode-forwards">
              <div className="flex items-center justify-between">
                ...
                <Link href="/challenges" className="text-sm font-bold text-[#E2D1FE] hover:text-white flex items-center gap-1 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded">
              ...
              {quests && quests.length > 0 ? (
                <div className="grid sm:grid-cols-2 gap-5">
                  {quests.map(quest => (
                    <Card key={quest.id} className="bg-black/20 border-white/10 backdrop-blur-xl flex flex-col justify-between hover:bg-white/[0.05] hover:border-white/20 hover:shadow-[0_8px_32px_rgba(134,56,205,0.15)] transition-all duration-300 rounded-2xl group">
                      ...
                      <CardContent className="pt-0">
                        <Button tabIndex={0} className="w-full font-bold bg-white/10 hover:bg-white/20 text-foreground border-transparent backdrop-blur-md transition-all rounded-xl h-11 focus-visible:ring-2 focus-visible:ring-accent" asChild>
            ...
            <section className="space-y-5 animate-fade-in-up [animation-delay:400ms] opacity-0 fill-mode-forwards">
              <div className="flex items-center justify-between">
                ...
                <Link href="/courses" className="text-sm font-bold text-[#E2D1FE] hover:text-white flex items-center gap-1 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded">
              ...
              {courses && courses.length > 0 ? (
                <div className="grid sm:grid-cols-2 gap-5">
                  {courses.map(course => (
                    <Card key={course.id} className="bg-black/20 border-white/10 backdrop-blur-xl flex flex-col justify-between hover:bg-white/[0.05] hover:border-white/20 hover:shadow-[0_8px_32px_rgba(134,56,205,0.15)] transition-all duration-300 rounded-2xl group">
                      <CardHeader className="pb-4">
                      ...
                      <CardContent className="pt-0">
                        <Button tabIndex={0} className="w-full font-bold bg-transparent border border-white/20 hover:bg-white/10 text-foreground transition-all rounded-xl h-11 focus-visible:ring-2 focus-visible:ring-accent" asChild>
                          <Link href={`/courses/${course.slug || course.id}`}>View Course</Link>
                        </Button>
                      </CardContent>
                    </Card>
                  ))}
          ...
          {/* Sidebar Column (Announcements, Next Masterclass & Real Backend Activity Feed) */}
          <div className="space-y-6">
              ...
              <CardContent className="pt-5">
                {nextSession ? (
                  <div className="space-y-5">
                    ...
                    <Button tabIndex={0} className="w-full h-11 bg-brand-gradient hover:brightness-110 text-foreground border-none font-bold shadow-lg transition-all rounded-xl accent-glow focus-visible:ring-2 focus-visible:ring-accent" asChild>
            ...
            {/* Real Backend Activity Feed Card */}
                ...
                {recentXpEvents && recentXpEvents.length > 0 ? (
                  <div className="space-y-3">
                    {recentXpEvents.map((event) => (
                      <div key={event.id} className="flex items-center justify-between text-xs bg-white/[0.02] border border-white/5 p-3 rounded-xl">
                        <span className="capitalize text-[#E2D1FE]/90 font-medium">
                          Earned XP from <strong className="text-white">{event.source_type}</strong>
                        </span>
                        <Badge className="bg-accent/20 text-accent font-bold border-accent/30">
                          +{event.amount} XP
                        </Badge>
                      </div>
                    ))}
                  ...
                  <div className="text-xs text-[#E2D1FE]/70 text-center py-8 border border-dashed border-white/20 rounded-xl bg-white/5 px-6 font-medium leading-relaxed">
...
function DashboardSkeleton() {
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12 animate-pulse">
      <div className="rounded-3xl bg-black/20 p-10 h-48 border border-white/10 flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="h-48 bg-black/20 rounded-2xl border border-white/10"></div>
          <div className="h-48 bg-black/20 rounded-2xl border border-white/10"></div>
        </div>
      </div>
    </div>
