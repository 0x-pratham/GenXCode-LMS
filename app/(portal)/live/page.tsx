...
import { Video, Calendar, Clock, CheckCircle2, XCircle, ExternalLink, Loader2 } from "lucide-react";
...
async function getLiveSessionsData(userId: string) {
  ...
  // 1. Fetch live sessions and user attendance records CONCURRENTLY
  // RLS policy "members view published sessions" automatically restricts non-staff to published sessions only,
  // while allowing admins to view drafts[cite: 12].
  const [
  ...
  // Split sessions into Upcoming and Past based on current time
  const now = new Date();
  const allSessions = sessions || [];
  
  const upcomingSessions = allSessions.filter(s => new Date(s.starts_at) >= now);
  // Reverse past sessions so the most recent past session shows at the top
  const pastSessions = allSessions.filter(s => new Date(s.starts_at) < now).reverse();

  return { upcomingSessions, pastSessions, attendanceMap };
}
...
export default async function LiveSessionsPage() {
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-12 relative z-10 focus:outline-none" tabIndex={0}>
      ...
      <Suspense fallback={<LiveSessionsSkeleton />}>
        <LiveSessionsContent userId={user.id} />
      </Suspense>
...
async function LiveSessionsContent({ userId }: { userId: string }) {
  const { upcomingSessions, pastSessions, attendanceMap } = await getLiveSessionsData(userId);
    ...
    <Tabs defaultValue="upcoming" className="w-full">
      ...
      <div className="animate-fade-in-up [animation-delay:200ms] opacity-0 fill-mode-forwards mb-8 inline-block">
        <TabsList className="bg-black/40 border border-white/10 p-1.5 rounded-2xl backdrop-blur-xl h-auto">
          <TabsTrigger 
            value="upcoming" 
            className="rounded-xl px-6 py-2.5 text-sm font-bold data-[state=active]:bg-white/10 data-[state=active]:text-foreground data-[state=active]:shadow-md text-[#E2D1FE]/60 transition-all focus-visible:ring-2 focus-visible:ring-accent"
          >
            Upcoming Classes
          </TabsTrigger>
          <TabsTrigger 
...
function LiveSessionsSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      <div className="w-48 h-10 bg-white/10 rounded-2xl mb-8"></div>
      <div className="grid gap-6 md:grid-cols-2">
        {[1, 2].map((i) => (
          <div key={i} className="bg-black/20 border border-white/10 rounded-3xl h-[320px] p-6 flex flex-col">
            ...
            <div className="w-full h-16 bg-white/5 rounded mb-6"></div>
