...
export default async function LeaderboardPage() {
  ...
  // Admin Client to bypass RLS and fetch all active profiles for ranking
  const supabaseAdmin = createAdminClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!
  );

  // Fetch profiles (added role and must_change_password for the Sidebar)
  const [
  ...
  // Aggregate total XP from xp_events table
  ...
  // ==========================================
  // SCENARIO 1: PUBLIC USER (NOT LOGGED IN) -> Show Navbar & Footer
  // ==========================================
  if (!user) {
    // Map top users and ensure avatar_url is passed
  ...
  // ==========================================
  // SCENARIO 2: AUTHENTICATED USER (PORTAL) -> Show Sidebar & Portal UI
  // ==========================================
  
  // Extract logged-in user's data for the Sidebar props
  let userRole = "student";
  let mustChangePassword = false;
  if (profiles) {
    const currentUserProfile = profiles.find(p => p.id === user.id);
    if (currentUserProfile) {
      userRole = currentUserProfile.role || "student";
      mustChangePassword = currentUserProfile.must_change_password || false;
    }
  }
